...
bhv_code(10,'Fix',70, 'Step7',100,'Reward'); 
...
cd(char(behavior_choosen)); %go the folder of the relationship choosen

list_images = dir('*.bmp*'); %list of all the images in the folder
rank = TrialRecord.User.pictures_left7(randi(length(TrialRecord.User.pictures_left7), 1)); %choose the number of the picture to show
...
cd control %go to the control subfolder to take the distractor pictures
list_distractor = dir('*.bmp*');
distractors = randi(size(list_distractor,1), 1,2); 

im_incor1 = ImageGraphic(touch_); %get a random picture from the nth unrelated behavior
im_incor1.List = {char(list_distractor(distractors(1)).name), pos(m(2),:), 1.1}; 
...
im_incor2.List = {char(list_distractor(distractors(2)).name), pos(m(3),:), 1.1}; 
% present this picture in another random position from the 5 possible
...
% relationship along with 2 other unrelated behaviors in random positions 
% of the screen (test2 phase)
...
% reward
if 0==error_type || 5==error_type %if the subject clicks on a picture
    TrialRecord.User.pictures_left7(TrialRecord.User.pictures_left7 == rank) = []; %delete the element that has been choosen
